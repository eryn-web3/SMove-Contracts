...
// added functionnalities by the smove's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when SMO mining ends.
    uint256 public bonusEndBlock;
    // Deposited amount SMO in SmartChef
	...
	uint256 lpSupply = depositedSmo;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 smoReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accSmoPerShare = accSmoPerShare.add(smoReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accSmoPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedSmo = depositedSmo.add(_amount).sub(depositFee).sub(transferTax);
            }else{
                uint256 transferTax = _amount.mul(smo.transferTaxRate()).div(10000);
                user.amount = user.amount.add(_amount).sub(transferTax);
                depositedSmo = depositedSmo.add(_amount).sub(transferTax);
            }
        }        
    ...
    // Withdraw SMO tokens from STAKING.
